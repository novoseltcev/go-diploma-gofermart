...
func (r *repository) GetBalance(ctx context.Context, userID uint64) (result models.Money, err error) {
	return result, r.tx.GetContext(ctx, &result, "SELECT balance FROM users WHERE id = $1", userID)
}
...
func (r *repository) GetUserWithdrawals(ctx context.Context, userID uint64) (result []models.Withdraw, err error) {
	err = r.tx.SelectContext(ctx, &result, `SELECT "order", sum FROM withdrawals WHERE user_id = $1 ORDER BY processed_at DESC`, userID)
	return result, err
}
